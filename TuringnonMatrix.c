...
int main(){
	int i;//i e j são iteradores, **MatrizFunção é a matriz cujo par (x,y) identifica a existência de caminhos entre o estado Qx e Qy (posicao [i][j] == 1).
	...
	lerCadeias(dados, 0, total);//linha 117
	return 0;
...
int testaCadeia(char *cadeia, dadosTuring dados, estado *total){
	...
	int currState = 0, j, currCharIndex = 0;//currState é o estado atual da máquina (iniciado em q0 por padrão), i e j iteradores, currCharIndex é o indíce na fita para atualizar currChar;
